"""
Gold Price Data Fetcher using yfinance
Fetches daily XAU/USD price data from 2021-01-01 to present
...
def fetch_gold_price(start_date='2021-01-01', end_date=None, incremental=True):
    """
    Fetch daily gold price data (XAU/USD) using yfinance
    ...
    # Gold futures ticker: GC=F (most reliable for gold price)
    ...
    if df.empty:
        print("Warning: No new data available from GC=F")
...
def clean_gold_price_data(df, raw_data_str):
    """
...
def save_gold_price_data(df, output_file='clean_observations.csv'):
    """
    Save cleaned gold price data to CSV file
    
    Parameters:
    -----------
    df : pd.DataFrame
        DataFrame with cleaned gold price data
    output_file : str
...
def main():
    """Main function to fetch and save gold price data"""
    
    print("=" * 60)
    print("Gold Price Data Fetcher - Incremental Update")
    print("=" * 60)
    
    # Fetch gold price data (incremental update)
    df_raw = fetch_gold_price(start_date='2021-01-01', incremental=True)
    ...
    if df_raw is not None and not df_raw.empty:
        ...
        df_clean = clean_gold_price_data(df_raw, raw_data_str)
        ...
        if df_clean is not None and not df_clean.empty:
            ...
            try:
                with DatabaseSession("gold_price.py") as db:
                    db.save(df_clean)
