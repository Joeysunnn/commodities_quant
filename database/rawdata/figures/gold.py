...
gld_holdings = pivot_metric(df, 'gld_holdings_oz')
...
fig_g1.update_layout(
    title=dict(text='G1: Gold Spot Price (YFINANCE)', x=0.5, xanchor='center', font=dict(size=18)),
    xaxis_title='Date',
...
add_range_selector(fig_g1)

# ============================================================================
# 图 G2: 三地可视库存（分频处理 - 使用子图）
# ============================================================================
print("G2: 三地库存子图...")
...
# 子图C: GLD (日频)
fig_g2.add_trace(go.Scatter(
    x=gld_holdings.index,
    y=gld_holdings.values,
    name='GLD',
...
fig_g2.update_layout(
    title=dict(text='G2: Three-Region Visible Inventory (Multi-Frequency)', x=0.5, xanchor='center', font=dict(size=18)),
    template='plotly_white',
    height=900,
    hovermode='x unified',
    showlegend=True
...
fig_g2.update_yaxes(title_text='oz', row=1, col=1)
fig_g2.update_yaxes(title_text='oz', row=2, col=1)
fig_g2.update_yaxes(title_text='oz', row=3, col=1)
fig_g2.update_xaxes(title_text='Date', row=3, col=1)
...
# ============================================================================
# 图 G3: 结构紧缺度（Registered/Total 比例）
# ============================================================================
print("G3: 结构紧缺度...")
fig_g3 = go.Figure()
...
fig_g3.update_layout(
    title=dict(text='G3: COMEX Structural Tightness (Registered/Total)', x=0.5, xanchor='center', font=dict(size=18)),
    xaxis_title='Date',
...
add_range_selector(fig_g3)

# ============================================================================
# 图 G4: 地理再分配指标（COMEX/LBMA 比率）
# ============================================================================
print("G4: 地理再分配指标...")
fig_g4 = go.Figure()
...
fig_g4.update_layout(
    title=dict(text='G4: Geographic Redistribution (COMEX/LBMA Ratio, Monthly)', x=0.5, xanchor='center', font=dict(size=18)),
    xaxis_title='Date',
...
# 计算 4 周变化量
comex_delta_4w = comex_total.diff(periods=28).dropna()  # 4周 ≈ 28天
gld_delta_4w = gld_holdings.diff(periods=28).dropna()
...
fig_g5.add_trace(go.Bar(
    x=gld_delta_4w.index,
    y=gld_delta_4w.values,
    name='Δ4W GLD Holdings',
    marker_color='green',
    opacity=0.7
))

# 添加零线
fig_g5.add_hline(y=0, line_dash="solid", line_color="black", line_width=1)

fig_g5.update_layout(
    title=dict(text='G5: Inventory Flow/Inflection (4-Week Change)', x=0.5, xanchor='center', font=dict(size=18)),
    xaxis_title='Date',
...
print("\n✓ 完成！5 张 GOLD 图表已生成")
print("\n图表说明：")
print("G1: 金价走势（未来可叠加实际利率）")
print("G2: 三地库存水平（LBMA月频 + COMEX日频 + GLD日频，分层展示）")
print("G3: 结构紧缺度（Registered/Total比例 + 绝对库存）")
print("G4: 地理再分配（COMEX/LBMA比率，月度对齐）")
print("G5: 库存流量（4周变化量，识别拐点）")