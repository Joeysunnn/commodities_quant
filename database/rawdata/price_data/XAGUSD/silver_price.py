"""
Silver Price Data Fetcher using yfinance
Fetches daily XAG/USD price data from 2021-01-01 to present
...
def fetch_silver_price(start_date='2021-01-01', end_date=None, incremental=True):
    """
    Fetch daily silver price data (XAG/USD) using yfinance
    ...
    --------
    pd.DataFrame
        DataFrame with silver price data
    """
    ...
    # Silver futures ticker: SI=F (most reliable for silver price)
    ...
    if df.empty:
        print("Warning: No new data available from SI=F")
        print("This usually means the market is closed or no trading on these dates")
        return None
...
def clean_silver_price_data(df, raw_data_str):
    """
...
def save_silver_price_data(df, output_file='clean_observations.csv'):
    """
    Save cleaned silver price data to CSV file
    
    Parameters:
    -----------
    df : pd.DataFrame
        DataFrame with cleaned silver price data
    output_file : str
...
def main():
    """Main function to fetch and save silver price data"""
    
    print("=" * 60)
    print("Silver Price Data Fetcher - Incremental Update")
    print("=" * 60)
    
    # Fetch silver price data (incremental update)
    df_raw = fetch_silver_price(start_date='2021-01-01', incremental=True)
    ...
    if df_raw is not None and not df_raw.empty:
        ...
        df_clean = clean_silver_price_data(df_raw, raw_data_str)
        ...
        if df_clean is not None and not df_clean.empty:
            ...
            try:
                with DatabaseSession("silver_price.py") as db:
                    db.save(df_clean)
